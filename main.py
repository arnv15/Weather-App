...
def update_imag():
    ...
    if weather in ("Sun", "Sunny", "Clear"):
        window["-IMAGE-"].update("images/sunny.png")

    # periodic clouds
    if weather in ("Clear with periodic clouds", "Mostly Sunny"):
        window["-IMAGE-"].update("images/sunny_s_cloudy.png")

    # partly cloudy
    if weather in ("Partly Sunny", "Partly cloudy", "Cloudy periodically clear"):
        window["-IMAGE-"].update("images/partly-cloudy.png")

    # cloudy
    if weather in ("Mostly cloudy", "Cloudy", "Overcast"):
        window["-IMAGE-"].update("images/cloudy.png")

    # light rain
    if weather in ("Light rain", "Chance of Rain"):
        window["-IMAGE-"].update("images/rain_light.png")

    # rain
    if weather in ("Rain and Snow", "Hail"):
        window["-IMAGE-"].update("images/snow_s_rain.png")
    ...
    # windy
    if weather in ("Windy"):
        window["-IMAGE-"].update("images/windy.png")
